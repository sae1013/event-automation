...
function EventPage(props) {
  ...
  if (isLoading) {
    return (
      <div className={styles.spinner__wrap}>
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.eventHeader}>
        ...
        <p
          className={styles.eventDate}>{`${parseDateToString(eventData?.eventStartDate)} ~ ${parseDateToString(eventData?.eventEndDate)}`}</p>
      </div>

      <div className={styles.eventWrapper}>
        <img onLoad={() => setIsImgLoaded(true)} ref={imageRef} src={eventData?.eventImageUrl} alt={'eventPage'}
             useMap={'#imageMap'} />
        <map name={'imageMap'} ref={areaRef}>
          {eventData?.eventArea.map((area, idx) => {
            return (
              <area key={idx} shape='rect' alt='eventBtn' data-type={area.eventType} data-contents={area.couponNumber}
                    onClick={onClickAreaHandler} />
            )
          })}
        </map>
      </div>
